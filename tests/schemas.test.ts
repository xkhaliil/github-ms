...
const valid = {
  description:
    "CLI that converts Postman collections into runnable pytest suites",
...
describe("ProposalSchema", () => {
  ...
  it("rejects fewer than three topics", () => {
    expect(
      ProposalSchema.safeParse({ ...valid, topics: ["python", "cli"] }).success,
    ).toBe(false);
  });
  ...
  it("rejects topics with uppercase or spaces", () => {
    expect(
      ProposalSchema.safeParse({ ...valid, topics: ["Python", "cli", "x"] })
        .success,
    ).toBe(false);
    expect(
      ProposalSchema.safeParse({
        ...valid,
        topics: ["machine learning", "cli", "x"],
      }).success,
    ).toBe(false);
  });

  it("rejects a README too short to be real", () => {
    expect(ProposalSchema.safeParse({ ...valid, readme: "# hi" }).success).toBe(
      false,
    );
  });
});
