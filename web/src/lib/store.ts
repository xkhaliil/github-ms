...
export const writeInventory = (inv: Inventory) =>
  idbSet(STORE_KV, INVENTORY, inv).then(() => {});

export const readAudit = () => idbGet<AccountAudit>(STORE_KV, AUDIT);
export const writeAudit = (audit: AccountAudit) =>
  idbSet(STORE_KV, AUDIT, audit).then(() => {});
...
export const readProposal = (name: string) =>
  idbGet<RepoProposal>(STORE_PROPOSALS, name);
...
export const readEvidence = <T>(name: string) =>
  idbGet<T>(STORE_EVIDENCE, name);
...
export async function clearAllData(): Promise<void> {
  await Promise.all([
    idbClear(STORE_KV),
    idbClear(STORE_PROPOSALS),
    idbClear(STORE_EVIDENCE),
  ]);
}
